...
def analyze_placement(df_clean: pd.DataFrame) -> None:
    ...
    with col1:
        selected_activities = st.multiselect("选择广告活动查看每日明细", all_activities, default=[],
                                             key="placement_activity_filter")
    with col2:
        selected_placements = st.multiselect("选择广告位查看每日明细", all_placements, default=[],
                                             key="placement_filter")
    # 过滤汇总表
    filtered_summary = [
        item for item in result["summary"]
    ...
    if '点击率(整体)' in summary_df.columns:
        summary_df['点击率(整体)'] = summary_df['点击率(整体)'].apply(lambda x: f"{x:.1%}" if pd.notna(x) else '-')
    if 'ACOS(整体)' in summary_df.columns:
        summary_df['ACOS(整体)'] = summary_df['ACOS(整体)'].apply(lambda x: f"{x:.1%}" if pd.notna(x) else '-')
    ...
    if filtered_summary:
        ...
        for act, group in filtered_summary_df.groupby('广告活动名称'):
            ...
            worst = group_sorted.tail(1)
            for _, row in best.iterrows():
                top_by_activity.append({'广告活动名称': act, '放置': row['放置'], '整体ACOS': row['整体ACOS']})
            for _, row in worst.iterrows():
                worst_by_activity.append({'广告活动名称': act, '放置': row['放置'], '整体ACOS': row['整体ACOS']})
    ...
    # 每日明细折叠面板：仅对筛选出的活动和广告位组合创建 expander
    if result["daily_details"] and selected_activities and selected_placements:
        ...
        with st.expander("📅 各广告活动+广告位每日明细（点击展开）"):
            for (act, place) in [(a, p) for a in selected_activities for p in selected_placements]:
                ...
                with st.expander(
                        f"📁 {act} - {place} | 总花费: {total_spend:.2f} | 总销售额: {total_sales:.2f} | ACOS: {avg_acos:.1%}"):
                    display_daily = daily_df.copy()
                    display_daily['点击率'] = display_daily['点击率'].apply(
                        lambda x: f"{x:.1%}" if pd.notna(x) else '-')
                    display_daily['ACOS'] = display_daily['ACOS'].apply(lambda x: f"{x:.1%}" if pd.notna(x) else '-')
